...
jstring Java_com_silkandroid_silkactivity_stringFromJNI( JNIEnv* env,  jobject thiz , jstring inputName, jstring outputName1, jstring outputName2)
{
    char*  str;
    uint64_t features;
    char buffer[512];
    ...
    const char* input;
    input = env->GetStringUTFChars(inputName, false);
    const char* output1;
    output1 = env->GetStringUTFChars(outputName1, false);
    const char* output2;
    output2 = env->GetStringUTFChars(outputName2, false);
